...
The hybrid retriever is always enabled; ensure the assets listed below exist (the toy versions in `data/processed/` let you run immediately).
...
* To stay in mock/demo mode, no action is required—tests and the API will read from `data/examples/`.
...
## Hybrid retriever assets (required)

The API and tests always load the hybrid retriever. The repository ships toy assets under `data/processed/` so you can run immediately, but rerun the steps below whenever you ingest new data.

1. Build imagery metadata (if you only have GeoTIFF scenes)
```bash
python scripts/extract_imagery_metadata.py \
  --imagery-dir data/raw/imagery \
  --pattern '*ColorBalance.tif' \
  --auto-zip \
  --output data/processed/imagery_tiles.parquet
```
(Add `--default-zip` or `--zip-map` if you already know the ZIP codes.)

2. Build the unified text corpus
```bash
python scripts/build_text_corpus.py --data-dir data --output data/processed/text_corpus.jsonl
```

3. Create the embedding index
```bash
...
4. Optionally enable reranking with `HARVEY_ENABLE_RERANKER=true`.
...
curl -X POST http://localhost:8000/chat \
  -H "Content-Type: application/json" \
  -d '{"message":"Give me a Harvey damage summary for zip 77002 between Aug 28 and Sept 3 2017"}'
```

The chat endpoint interprets the sentence, shows the resolved query, and returns the same structured answer payload.
...
- `scripts/process_nfip_claims.py`
