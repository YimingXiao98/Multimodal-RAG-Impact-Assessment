...
class HybridTextRetriever:
    ...
    def __init__(self, locator: DataLocator, settings) -> None:
        ...
        self.doc_map: Dict[str, dict] = {
            doc["doc_id"]: doc for doc in self.docs}
        if not self.doc_map:
            raise RuntimeError(
                "Hybrid corpus is empty; rerun scripts/build_text_corpus.py")

        self.embedding_doc_ids = self._load_ids(self.ids_path)
        self.meta = self._load_meta(self.meta_path)

        self.embedder = SentenceTransformer(
            self.meta["model"], device=self.device)
        self.faiss_index = faiss.read_index(str(self.index_path))

        bm25_tokens = [self.doc_map[doc_id]["tokens"]
                       for doc_id in self.embedding_doc_ids if doc_id in self.doc_map]
        self.bm25_docs = [self.doc_map[doc_id]
                          for doc_id in self.embedding_doc_ids if doc_id in self.doc_map]
        if not bm25_tokens:
            raise RuntimeError(
                "No overlapping docs between corpus and embedding index")
        ...
        if self.enable_reranker:
            rerank_model = getattr(
                settings, "hybrid_reranker_model", "BAAI/bge-reranker-base")
    ...
    def _resolve_path(self, path_value: Path | str) -> Path:
        ...
        if not path.exists():
            raise RuntimeError(
                f"Required file not found: {path}. Run the corpus/index scripts first.")
        return path
    ...
    def _load_docs(self, path: Path) -> List[dict]:
        ...
        with path.open("r", encoding="utf-8") as fh:
            for line in fh:
                ...
                doc = json.loads(line)
                doc.setdefault(
                    "tokens", (doc.get("text") or "").lower().split())
    ...
    def _dense_scores(self, query_text: str, allowed_ids: List[str]) -> Dict[str, float]:
        ...
        query_vec = self.embedder.encode(
            [query_text], convert_to_numpy=True, normalize_embeddings=True).astype("float32")
        distances, indices = self.faiss_index.search(
            query_vec, self.dense_top_k)
        allowed_set = set(allowed_ids)
    ...
    def _rerank(self, query_text: str, doc_ids: List[str]) -> List[str]:
        if not doc_ids or not self.cross_encoder:
            return doc_ids
        pairs = [(query_text, self.doc_map[doc_id]["text"])
                 for doc_id in doc_ids if doc_id in self.doc_map]
        if not pairs:
            return doc_ids
        scores = self.cross_encoder.predict(pairs)
        scored = sorted(zip([doc_id for doc_id in doc_ids if doc_id in self.doc_map],
    ...
    def _resolve_table_path(self, name: str) -> Path:
        ...
        if example.exists():
            ...
            return example
        raise RuntimeError(
            f"Required table '{name}' not found in processed or example directories.")
